import { CONSTITUTION } from '../writer/constitution'
...
type Env = Pick<Cloudflare.Env, 'DB' | 'TRANSCRIPTS' | 'MASTER_KEY'>
...
const TOOLS = [
  ...
  },
  {
    name: 'writing_guide',
    description:
      'The editorial law this workspace writes under. Fetch it before drafting a post from any call, and follow it exactly — including its permission to conclude there is no post in the material.',
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'save_post',
    description:
      'Save a finished post to TallTrack so it appears on the workspace front door. Only save a post the person has SEEN and approved in this conversation — never save drafts on your own initiative.',
    inputSchema: {
      type: 'object',
      properties: {
...
async function runTool(
  ...
  if (name === 'this_week') {
    ...
    return text(
      JSON.stringify(
        calls.map((c) => ({ id: c.id, title: c.title ?? 'Untitled call', date: c.occurred_at })),
        null,
        2,
      ),
    )
  ...
  if (name === 'read_call') {
    ...
    return text(`# ${call.title} — ${call.occurredAt}\n\n${call.transcript}`)
  }
  ...
  if (name === 'save_post') {
    ...
    return text(`Saved. It's on the TallTrack front door now (${id}).`)
  }
